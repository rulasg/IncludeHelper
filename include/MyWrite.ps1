# {"Version":"1.7.0","Date":"2026-09-08","Source":"SalesHelper","HeaderVersion":1}
...
function Write-MyHost {
    ...
    param(
        ...
        [Parameter()][switch]$InNewLine
    )

    # Add starting new line if InNewLine is specified
    if ($InNewLine) { $Message = "`n" + $Message }
...
function Write-MyDot{
    [cmdletbinding()]
    [Alias("Write-Dot")]
    param(
        [Parameter()][object]$Object,
        [Parameter()][int]$objectDepth = 10,
        [Parameter()][switch]$ObjectNotCompress,
        
        [Parameter(ValueFromPipeline)][string]$Message,
        [Parameter(Position = 0)][string]$ForegroundColor = $OUTPUT_COLOR,
    ...
    process {
        ...
        if(-not (Test-MyDebug -section "dot")) {
            # Write a dot with the specifications
            "." | Write-MyHost -ForegroundColor $ForegroundColor -NoNewLine:$(-not $NewLine) -InNewLine:$InNewLine
        } else {
            # Write Debug with the specifications
            Write-MyDebug -Section "dot" -Message $Message -Object $Object -objectDepth $objectDepth -ObjectNotCompress:$ObjectNotCompress -InNewLine:$InNewLine
        }
    }
}
...
function Write-MyDebug {
    ...
    process{
        ...
        if (Test-MyDebug -section $section) {
            ...
            if ($Object) {
                $compress = -not $ObjectNotCompress
                $objString = $Object | Get-ObjetString -Depth $objectDepth -Compress:$compress
...
function Enable-ModuleNameDebug{
    param(
        ...
        [Parameter()][switch]$DoNotClearHost
    )
...
function Get-ObjetString {
    ...
    process{
        ...
        return $Object | ConvertTo-Json -Depth $Depth -ErrorAction SilentlyContinue -Compress:$Compress
    }
}
